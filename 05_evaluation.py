...
print(f"retrieval_metrics.csv disimpan di: {retrieval_csv}")
...
if "actual_solution" in predictions_df.columns:
    ...
    print(f"prediction_metrics.csv disimpan di: {pred_eval_csv}")
else:
    print("Kolom 'actual_solution' tidak ditemukan di predictions.csv — evaluasi prediksi dilewati.")
...
def show_errors(eval_df, queries, max_display=5):
    ...
    if display_count == 0:
        print("Tidak ada retrieval yang gagal.")
        return

    print(f"Menampilkan {display_count} dari {len(error_queries)} kasus error:")
